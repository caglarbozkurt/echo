...
```bash
curl -X POST https://echo.to/api/publish \
...
Response:
```json
{ "slug": "x9k2abcd", "url": "https://echo.to/d/x9k2abcd" }
```
...
| `password` | no       | If set, viewers see a password gate. If omitted, document is public.  |
| `title`    | no       | Display title shown on the password gate and metadata.                 |

## Behavior

- Markdown is server-rendered with sanitization (DOMPurify). Raw `<script>` tags in MD are stripped.
...
> ✓ Published to `https://echo.to/d/x9k2abcd`
